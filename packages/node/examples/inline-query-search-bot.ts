...
const KNOWLEDGE_BASE = [
  {
    ...
    desc: "Handbook and reference",
  },
  {
    id: "2",
    title: "Telegram Bot API",
    url: "https://core.telegram.org/bots/api",
    desc: "Official Telegram Bot specification",
  },
  {
    id: "3",
    title: "telebot-ts GitHub",
    url: "https://github.com",
    desc: "Native zero-dependency Telegram Bot framework",
  },
];
...
app.addHandler(
  new InlineQueryHandler(async (update: Update, context: CallbackContext) => {
    ...
    const results = filtered.map((item) => {
      ...
      }).text(`${bold(item.title)}\n${item.desc}\n\nLink: ${item.url}`, { parse_mode: "HTML" });
    });
