...
6. **The method-parameter layer is measured the same way.** `audit:fidelity` also diffs `oracle.methods[*].params` against what each package can actually send, and a param only counts when it is enumerable in that language's own idiom: node — a property on the named options interface (or an inline object type) that reaches `this.request("wire", …)`; go — a `json:"name"` tag on `types.XOptions`, a key in the `map[string]any` payload, or one in an anonymous payload struct; python — an explicit keyword argument on the `async def` reaching `clean_payload(...)`. A python `**kwargs`, or a node `options: Record<string, unknown>` that swallows the params, makes the method *unmeasurable* — the audit never invents a key list it cannot read, so routing arguments through an untyped bag is a regression, not a shortcut.
